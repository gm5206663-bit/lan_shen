#!/usr/bin/env python3
"""
SOUL LAND UNIVERSAL KIT — the machine gate.  **UNIFIED, v2.2, 2026-09-20**
**Lan Shen fork: upstream v2.2 verbatim + 2 LOCAL gates (8 panel field discipline,
9 en-dash year range). Rebased 2026-09-20 from a v1 fork. Gates 1-7 are the
kit's and must not be edited here; edit them upstream and rebase.**
...
LOCAL to this serial, prose/panel only (added by the fork, not upstream)
    8. panel field discipline -- no prose hiding inside the STATUS panel fence,
       where it would escape gates 1-3 and the word count
    9. the panel year range must use an en dash; a hyphen parses as a silent
       single year and corrupts gate 5's contiguity arithmetic
...
# --- LOCAL to the Lan Shen serial (not upstream) -----------------------------
# A panel line is either "FIELD   : value" or an indented continuation of the
# field above it. Used by local gate 8.
PANEL_FIELD = re.compile(r"[A-Z][A-Z0-9 /\-]{0,22}\s*:")
...
def verify_file(path):
    ...
    # --- LOCAL gate 8: panel field discipline --------------------------------
    # Every fenced block is apparatus and is therefore EXEMPT from gates 1-3.
    # That exemption is correct for a status panel and dangerous for anything
    # else: prose that slips inside the fence disappears from the word count and
    # escapes the CJK, backslash-n and digit gates entirely. A panel line is
    # either "FIELD   : value" or an indented continuation of the field above.
    # Anything else is prose in hiding.
    #
    # SCOPE: chapter gate, like 3-5 and 7. It applies only when the first fenced
    # block is genuinely a status panel, i.e. it carries the marker. A plain code
    # fence in a law doc, a handoff or a README is not a panel and must not be
    # judged as one -- verify.py is a CHAPTER gate and misbehaves on kit files by
    # design (the kit's own verify.py fails the kit's own 09_AUDIT_LAW.md).
    if panel.strip() and MARKER in panel:
        stray = []
        for ln in panel.split(NL):
            if not ln.strip():
                continue
            if PANEL_FIELD.match(ln):
                continue
            if ln.lstrip().startswith(MARKER):   # the "STATUS - Chapter N" title line
                continue
            if ln[:1] in (" ", chr(9)):   # indented continuation of a field
                continue
            stray.append(ln.strip()[:60])
        if stray:
            failures.append(
                f"prose inside the panel fence ({len(stray)} line(s)) - it escapes the "
                f"digit/CJK/backslash-n gates and the word count: {stray[:3]}"
            )

    # --- LOCAL gate 9: the en dash is load-bearing ---------------------------
    # PANEL_RANGE wants "years 4\u20135" with an EN DASH (U+2013). A hyphen does
    # not match; PANEL_ANY then swallows the first number and silently reports
    # the range as 4-4. Gate 5 (contiguity) is computed from that range, so a
    # hyphen quietly corrupts the serial's timeline arithmetic with no error.
    m_hyphen = re.search(r"years (\d+)-(\d+)", panel) if MARKER in panel else None
    if m_hyphen and not PANEL_RANGE.search(panel):
        failures.append(
            f"panel year range uses a hyphen, not an en dash: "
            f"'years {m_hyphen.group(1)}-{m_hyphen.group(2)}' - the parser silently "
            f"reports it as {m_hyphen.group(1)}-{m_hyphen.group(1)} and gate 5 is then wrong"
        )
